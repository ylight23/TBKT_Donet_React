                ...
                if (tbktItem != null)
                    filter &= Global.FitlerBuilderTrangBi.Ne(x => x.Id, request.IDCapTren) &
                              Global.FitlerBuilderTrangBi.Regex(x => x.Id,
                                  "/^" + EscapeRegex(Utils.TruncateTrangBiId(tbktItem.Id)) + "[.].*/i");
                else
                {
                    filter &= Global.FitlerBuilderTrangBi.Ne(x => x.Id, request.IDCapTren) &
                              Global.FitlerBuilderTrangBi.Regex(x => x.Id,
        ...
        try
        {
            ...
            if (request.Items != null && request.Items.Any() && collection != null)
            {
                foreach (var item in request.Items)
                {
                    ...
                    if (item.Nhom == 1)
                    {
                        var isValid = Utils.IsValidTBKT1Id(item.Id, item.IDNganh);
                        if (!isValid.Value)
                        {
                            response.Success = false;
                    ...
                    if (request.IsNew)
                    {
                        ...
                        if (item.Nhom == 1)
                        {
                            ...
                            var idTruncated = Utils.TruncateTrangBiId(item.Id);
                        ...
                        else
                        {
                            ...
                            var newId = Utils.SearchTrangBiIndexNotUsed(item.IDCapTren, isDeXuat: isDeXuat, context.GetUserAdminOfficeID());
                    ...
                    else
                    {
                        ...
                        if (item.Nhom == 1)
                        {
                            oldId = Utils.TruncateTrangBiId(oldId);
                        }
                        ...
                        if (changeParent)
                        {
                            var newParent = !string.IsNullOrEmpty(newParentId) ? Utils.GetTrangBiItemById(newParentId) : null;
                            var newId = Utils.SearchTrangBiIndexNotUsed(newIDCapTren, isDeXuat: isDeXuat, officeId: context.GetUserAdminOfficeID());
                            item.Id = item.IDCapTren + "." + newId;
