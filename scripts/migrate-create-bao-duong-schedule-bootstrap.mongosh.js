/*
  Bootstrap migration for BaoDuongSchedule when legacy TrangBiLog source is empty.
  - Source: TrangBiNhom1, TrangBiNhom2
  - Target: BaoDuongSchedule
  - _id: GUID string
  - Idempotent by BootstrapKey

  Run:
    mongosh "mongodb://localhost:27017/quanly_dmcanbo" --file scripts/migrate-create-bao-duong-schedule-bootstrap.mongosh.js
*/

const TARGET = "BaoDuongSchedule";
...
function mapTrangBi(doc, nhom) {
  ...
  return {
    ...
    Parameters: {
      ...
      trang_thai_su_dung: strOrEmpty(p.trang_thai),
    },
  };
}
...
function ensureIndexSafe(coll, keys, options) {
  try {
    ...
    if (msg.includes("Index already exists")) {
      print(`[index-skip] ${options?.name || JSON.stringify(keys)} already exists`);
      return;
    }
    throw err;
  }
...
for (const members of chunk(all, GROUP_SIZE)) {
  ...
  schedules.push({
    ...
    TenLichBaoDuong: `Lich bao duong bootstrap ${idx}`,
    CanCu: "Khoi tao tu migration bootstrap",
    IdDonVi: idDonVi,
    TenDonVi: idDonVi,
    NguoiPhuTrach: "admin",
    ThoiGianLap: new Date(Date.UTC(year, month, 1, 8, 0, 0)),
    ThoiGianThucHien: new Date(Date.UTC(year, month, Math.min(5 + idx, 25), 8, 0, 0)),
    ThoiGianKetThuc: new Date(Date.UTC(year, month, Math.min(6 + idx, 26), 17, 0, 0)),
    NoiDungCongViec: "Bao duong dinh ky theo ke hoach",
    VatChatBaoDam: "Vat tu, phu tung theo dinh muc",
    KetQua: "Dang thuc hien",
    DsTrangBi: members,
    Parameters: {
      nguon_du_lieu: "bootstrap_from_trangbi",
      loai_migration: "schedule_first_seed",
  ...
  if (existed) {
    if (!DRY_RUN) {
      target.updateOne(
        ...
        {
          $set: {
            TenLichBaoDuong: doc.TenLichBaoDuong,
            CanCu: doc.CanCu,
            IdDonVi: doc.IdDonVi,
...
if (!DRY_RUN) {
  ensureIndexSafe(target, { BootstrapKey: 1 }, { name: "idx_baoduong_bootstrap_key", unique: true, sparse: true });
  ensureIndexSafe(target, { IdDonVi: 1 }, { name: "idx_baoduong_id_don_vi" });
  ensureIndexSafe(target, { "DsTrangBi.IdTrangBi": 1 }, { name: "idx_baoduong_ds_trang_bi_id" });
  ensureIndexSafe(target, { "DsTrangBi.NhomTrangBi": 1 }, { name: "idx_baoduong_ds_trang_bi_nhom" });
}
