...
public class DynamicMenuService(ILogger<DynamicMenuService> logger)
{
    ...
    private static readonly string[] ChuyenNganhFieldCandidates =
    [
        "IDChuyenNganhKT",
        "IdChuyenNganhKT",
        "IDChuyenNganh",
        ...
        ServerCallContext? context)
    {
        ...
        try
        {
            ...
            var cnFieldKey = ResolveChuyenNganhFieldKey(dsBson);
            ...
            if (!string.IsNullOrWhiteSpace(cnFieldKey))
            {
                readFilter &= ServiceMutationPolicy.BuildCnVisibilityFilter(context, cnFieldKey, true);
            }
        ...
        ServerCallContext? context)
    {
        ...
        try
        {
            ...
            if (operation == "delete")
            {
                ...
                var existingDeleteDoc = await collection
                    .Find(Builders<BsonDocument>.Filter.Eq("_id", deleteId))
                    .FirstOrDefaultAsync();
                if (existingDeleteDoc == null)
                {
                    response.Meta = ThamSoResponseFactory.Fail($"Khong tim thay ban ghi id '{deleteId}' de xoa");
                    return response;
                }

                var deleteCnId = ResolveChuyenNganhId(existingDeleteDoc, cnFieldKey);
                if (!string.IsNullOrWhiteSpace(cnFieldKey))
                {
                    ServiceMutationPolicy.RequireActOnCN(context, "delete", deleteCnId, true);
                }
            ...
            BsonDocument? existingDoc = null;
            if (!string.IsNullOrWhiteSpace(recordId))
            {
                existingDoc = await collection
                    .Find(Builders<BsonDocument>.Filter.Eq("_id", recordId))
                    .FirstOrDefaultAsync();
            }

            var existingCnId = ResolveChuyenNganhId(existingDoc, cnFieldKey);
            var targetCnId = ResolveChuyenNganhId(sanitized, cnFieldKey) ?? existingCnId;
            if (!string.IsNullOrWhiteSpace(cnFieldKey))
            {
                if (string.IsNullOrWhiteSpace(targetCnId))
                {
                    response.Meta = ThamSoResponseFactory.Fail($"Datasource '{sourceKey}' yeu cau truong chuyen nganh '{cnFieldKey}'");
                    return response;
                }

                var action = existingDoc != null ? "edit" : "add";
                ServiceMutationPolicy.RequireActOnCN(context, action, targetCnId, true);
                if (existingDoc != null &&
                    !string.IsNullOrWhiteSpace(existingCnId) &&
                    !string.Equals(existingCnId, targetCnId, StringComparison.OrdinalIgnoreCase))
                {
    ...
    private static string ResolveChuyenNganhFieldKey(BsonDocument dsBson)
    {
        var fields = dsBson.ArrayOr("Fields") ?? new BsonArray();
        foreach (var doc in fields.Documents())
        {
            ...
            if (ChuyenNganhFieldCandidates.Any(candidate => key.Equals(candidate, StringComparison.OrdinalIgnoreCase)))
            {
                return key;
            }
        }
    ...
    private static string? ResolveChuyenNganhId(BsonDocument? doc, string? preferredFieldKey)
    {
        if (doc == null)
            return null;

        if (!string.IsNullOrWhiteSpace(preferredFieldKey) &&
            doc.TryGetValue(preferredFieldKey, out var preferredValue) &&
            !preferredValue.IsBsonNull)
        {
            var preferredText = preferredValue.ToString()?.Trim();
            if (!string.IsNullOrWhiteSpace(preferredText))
                return preferredText;
        }

        foreach (var candidate in ChuyenNganhFieldCandidates)
        {
            if (!doc.TryGetValue(candidate, out var value) || value.IsBsonNull)
                continue;
