...
interface FlatTreeItem {
    ...
    isExpanded: boolean;
    isLoading: boolean;
    rawNode: DanhMucTrangBiTree;
}

function buildFlatList(
    parentId: string,
    depth: number,
    cache: Map<string, DanhMucTrangBiTree[]>,
...
interface DanhMucTrangBiTreePanelProps {
    cn: string;
    selectedId: string;
...
const DanhMucTrangBiTreePanel: React.FC<DanhMucTrangBiTreePanelProps> = ({ cn, selectedId, onSelect }) => {
    const theme = useTheme();
    const [cache, setCache] = useState<Map<string, DanhMucTrangBiTree[]>>(new Map());
    const [expandedIds, setExpandedIds] = useState<Set<string>>(new Set());
    const [loadingRoot, setLoadingRoot] = useState(false);
    const [loadingIds, setLoadingIds] = useState<Set<string>>(new Set());
    const [searchText, setSearchText] = useState('');
    const [debouncedSearch, setDebouncedSearch] = useState('');
    const [searchResults, setSearchResults] = useState<DanhMucTrangBiTree[]>([]);
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            setSearchResults([]);
            try {
                const items = await danhMucTrangBiApi.getTreeChildren({ parentId: rootParentId });
                if (!cancelled) {
    ...
    useEffect(() => {
        ...
        const search = async () => {
            ...
            try {
                const items = await danhMucTrangBiApi.getTreeChildren({
    ...
    const loadChildren = useCallback(
        async (nodeId: string) => {
            ...
            try {
                const items = await danhMucTrangBiApi.getTreeChildren({ parentId: nodeId });
                            ...
                            return (
                                <Box
                                    ...
                                    sx={{
                                        ...
                                        pr: 0.5,
                                        cursor: 'pointer',
                                        ...getStripedRowSx(theme, vItem.index, isSelected),
...
export default DanhMucTrangBiTreePanel;
