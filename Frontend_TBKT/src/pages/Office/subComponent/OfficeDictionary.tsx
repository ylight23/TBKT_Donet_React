...
function computeVisibleList(
    ...
    const appendRows = (currentParentKey: string, currentDepth: number) => {
        ...
        for (const node of nodes) {
            ...
            const hasVisibleChildren = isLoading || !!(childNodes && childNodes.length > 0);

            rows.push({ node, depth: currentDepth, isExpanded, isLoading, canExpand, hasVisibleChildren });
...
const VirtualTreeRow = React.memo(function VirtualTreeRow({
    ...
    const { node, depth, isExpanded, canExpand, hasVisibleChildren } = row as TreeRow;
    ...
    return (
        ...
        >
            <TreeGuideLines
                ...
                hasExpandedChildren={isExpanded && hasVisibleChildren}
...
const OfficeDictionary = React.forwardRef<OfficeDictionaryRef, OfficeDictionaryProps>((props, ref) => {
    ...
    const getTreeRowMeta = useCallback((node: OfficeNode, depth: number): TreeRow => {
        ...
        return {
            ...
            hasVisibleChildren: !!loadingByParentRef.current[id] || !!(childNodes && childNodes.length > 0),
        };
    }, []);
    ...
    const visibleList = useMemo<VisibleRow[]>(() => {
        if (isSearchMode) {
            return (searchResults || []).map(node => ({
                node, depth: 0, isExpanded: false, isLoading: false, canExpand: false, hasVisibleChildren: false,
            }));
        }
                            ...
                            {virtualizer.getVirtualItems().map(virtualRow => {
                                ...
                                return (
                                    <VirtualTreeRow
                                        ...
                                        isDirectHovered={isTreeRow(row) && hoveredNodeId === getNodeId(row.node)}
