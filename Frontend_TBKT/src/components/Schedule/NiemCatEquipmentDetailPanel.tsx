...
export interface NiemCatEquipmentDetailPanelProps {
...
const INFO_KEYS = new Set(['ten_ke_hoach', 'loai_niem_cat', 'nguoi_thuc_hien', 'don_vi_thuc_hien', 'ngay_niem_cat', 'can_cu']);
...
const Stepper: React.FC<{
    ...
    const steps = [
        { label: 'Thong tin', done: Boolean(formData.ten_ke_hoach || formData.ngay_niem_cat) },
                            ...
                            <Stack direction="row" alignItems="center" spacing={0.65} onClick={() => onStepClick(stepNo)} sx={{ cursor: 'pointer', flexShrink: 0 }}>
                                <Box
                                    sx={{
                                        ...
                                        fontSize: 10,
                                        fontWeight: 900,
                                        bgcolor: step.done && !active ? '#FAEEDA' : active ? '#FAEEDA' : '#f5f4ef',
                                        color: step.done && !active ? '#6B3A08' : active ? '#6B3A08' : '#9a9a94',
                                        border: '1px solid',
                                        borderColor: step.done && !active ? '#EF9F27' : active ? '#EF9F27' : 'rgba(0,0,0,0.12)',
                                    }}
                                >
...
const NiemCatEquipmentDetailPanel: React.FC<NiemCatEquipmentDetailPanelProps> = ({
    equipment,
    value,
    onClose,
    onSave,
    ...
    useEffect(() => {
        ...
        setLoading(true);
        setError('');
        thamSoApi.getFieldSetsByKey(TRANG_BI_FIELD_SET_KEYS.NIEM_CAT_CHI_TIET)
            .then((data) => {
                if (mounted) setFieldSets(mergeFieldSets(data));
            })
            .catch((err) => {
                if (mounted) setError((err as Error).message || 'Khong tai duoc fieldset chi tiet niem cat.');
            })
            .finally(() => {
    ...
    const renderContent = (item: GeneralFieldSetItem | null, emptyText: string) => {
        ...
        return (
            <FieldSetGroup
                ...
                color={item.fieldSet.color || '#6B3A08'}
        ...
        <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', bgcolor: '#fff', minHeight: 0 }}>
            <Box sx={{ px: 1.5, py: 1.25, borderBottom: '1px solid', borderColor: 'divider', bgcolor: '#f5f4ef' }}>
                <Stack direction="row" spacing={1} alignItems="flex-start">
                    <Box sx={{ width: 34, height: 34, borderRadius: 1.25, bgcolor: '#FAEEDA', color: '#6B3A08', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        <WarehouseIcon fontSize="small" />
                    </Box>
                    <Box sx={{ minWidth: 0, flex: 1 }}>
                        <Typography sx={{ fontSize: 14, fontWeight: 900, color: '#1a1a18' }} noWrap>Chi tiet niem cat trang bi</Typography>
            ...
            {equipment && activeStep < 4 && (
                <Box sx={{ px: 1.5, py: 1, borderBottom: '1px solid', borderColor: 'divider', bgcolor: '#fbfbfb' }}>
                    <Stack direction="row" spacing={0.75} flexWrap="wrap" useFlexGap>
                        <Chip size="small" label={formData.loai_niem_cat || 'Niem cat'} sx={{ bgcolor: '#FAEEDA', color: '#6B3A08', height: 21, fontSize: 10 }} />
            ...
            <Box sx={{ flex: 1, minHeight: 0, overflow: 'auto', bgcolor: '#fff' }}>
                {activeStep === 1 && (
                    <PanelSection title="Thong tin chi tiet niem cat">
                        ...
                        {renderContent(contentByKey.info, 'Chua co field thong tin chi tiet niem cat.')}
                    </PanelSection>
                )}

                {activeStep === 2 && (
                    <PanelSection title="Noi dung thuc hien">
                        <Button size="small" variant="outlined" startIcon={<AddIcon />} onClick={() => setWorkRows((prev) => [...prev, createWorkRow()])} sx={{ mb: 1, borderStyle: 'dashed', color: '#6B3A08', borderColor: '#EF9F27' }}>
                            Them noi dung
                        </Button>
                ...
                {activeStep === 3 && (
                    <PanelSection title="Ket qua niem cat">
                        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 1, mb: 1.25 }}>
                            {[
                                { label: 'Ngay niem cat', value: formData.ngay_niem_cat || '--' },
                                { label: 'Nguoi thuc hien', value: formData.nguoi_thuc_hien || '--' },
                                { label: 'Don vi thuc hien', value: formData.don_vi_thuc_hien || '--' },
                        ...
                        {renderContent(contentByKey.result, 'Chua co field ket qua niem cat.')}
                    </PanelSection>
                )}

                {activeStep === 4 && (
                    <Box sx={{ p: 3, textAlign: 'center' }}>
                        <Box sx={{ width: 74, height: 74, borderRadius: '50%', bgcolor: '#FAEEDA', border: '1px solid #EF9F27', mx: 'auto', mb: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6B3A08', fontSize: 18, fontWeight: 900 }}>Hoan tat</Box>
                        <Typography sx={{ fontSize: 16, fontWeight: 900, mb: 1 }}>Phieu niem cat da duoc ghi nhan</Typography>
                        <Typography sx={{ fontSize: 12, color: '#5c5c57', mb: 2 }}>{equipment?.tenDanhMuc || 'Trang bi'} da co thong tin niem cat chi tiet</Typography>
                        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 1, textAlign: 'left' }}>
                            {[
                                { label: 'Loai niem cat', value: formData.loai_niem_cat || '--' },
...
export default NiemCatEquipmentDetailPanel;
